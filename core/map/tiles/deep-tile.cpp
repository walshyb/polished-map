...
static void fill_tile_pixel(unsigned char *tile, int x, int y, const unsigned char *rgb) {
...
void Deep_Tile::render_pixel(int x, int y, Palettes l, Hue h) {
	...
	for (int pi = 0; pi < NUM_PALETTES; pi++) {
		...
		const unsigned char *rgb = Color::color(l, p, h);
...
void Deep_Tile::draw_for_clipboard(int x, int y) {
	for (int ty = 0; ty < TILE_SIZE; ty++) {
		for (int tx = 0; tx < TILE_SIZE; tx++) {
			...
			unsigned char c = Color::hue_mono(_hues[ti]);
		}
	}
}
